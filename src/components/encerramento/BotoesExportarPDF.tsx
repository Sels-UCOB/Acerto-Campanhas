...
import { DocumentoPDF } from "@/lib/pdf/DocumentoPDF";
import type { TipoExportacao, DadosPDF, DiferencaCaixaCalc, GrupoCampanha } from "@/lib/pdf/types";
import styles from "./BotoesExportarPDF.module.css";
...
export function BotoesExportarPDF() {
  ...
  const montarDados = useCallback(
    (tipo: TipoExportacao): DadosPDF => {
      ...
      // Saldo inicial do módulo líderes
      const saldosLanc = calcularSaldos(lancamentos);
      const saldoInicialLider =
        saldosLanc.length > 0 ? saldosLanc[saldosLanc.length - 1] : 0;

      // Linhas do módulo de líderes
      const configs = config.lideres
      ...
      const caixaConfig = config.caixa.nome.trim()
        ? { nome: config.caixa.nome, auxilioPercentual: config.caixa.auxilioPercentual }
        : null;
      const linhasLider = gerarLinhasLider({
        configs,
        cartaBolsa,
        compraBonificada,
        jurosCampanha,
        salarioCaixa: config.caixa.salarioCaixa,
        caixaConfig,
      });

      // Resumos dos líderes
      const totalDevedores = calcularTotalDevedores(devedores);
      const lideres = Array.from({ length: numLideres }, (_, i) => config.lideres[i]).filter(
        (l) => l.nome.trim()
      );
      const resumosLideres = lideres.map((lider, idx) =>
        calcularResumoLider({
          lider,
      ...
      const totalDebitosLideres = arred(
        resumosLideres.reduce((s, r) => s + r.totalDebitos, 0)
      );
      ...
      const diferencaCaixa: DiferencaCaixaCalc = {
        saldoInicial: saldoInicialLider,
        salarioCaixa,
        fpc,
        juros,
        base,
        ...
        totalDebitosCaixa,
        totalDebitos,
        diferenca: arred(totalDebitos - base),
  ...
  const handleExportar = useCallback(
    async (tipo: TipoExportacao) => {
      ...
      try {
        ...
        const campanha =
          dados.config.tipoCampanha === "Outro"
            ? dados.config.tipoCampanhaOutro
            : dados.config.tipoCampanha;
        a.href = url;
  ...
  const botoes: { tipo: TipoExportacao; label: string; title: string }[] = [
    ...
    {
      tipo: "LIDERES",
      label: "Exportar Líderes",
      title: "Aba Líderes completa — 1 página por líder",
    },
