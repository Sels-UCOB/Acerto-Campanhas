        ...
        <div className="space-y-2">
          ...
          {devedores.map((d) => (
            <div key={d.id} className="flex items-center gap-2">
              <input className={cn(inputCls, "flex-1")} type="text" placeholder="Nome do colportor" value={d.nome} onChange={(e) => updateDevedor(d.id, { nome: e.target.value })} />
              <input
              ...
              />
              <button type="button" className="w-6 h-6 rounded-md flex items-center justify-center text-[#8B8FA8] hover:text-red-400 hover:bg-red-500/10 transition-colors text-base leading-none" onClick={() => removeDevedor(d.id)} title="Remover">×</button>
            </div>
          ))}
        </div>
      )}
